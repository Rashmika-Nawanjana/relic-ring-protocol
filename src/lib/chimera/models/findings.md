...
1. **Learned historical priors** — per-link trust parameters fitted from `link_telemetry.csv` (stored in `params.ts`, regenerated by `npm run train:models`).
2. **Live lie detection** — a runtime check that works on *any* link, including ones that only start lying on demo day.
